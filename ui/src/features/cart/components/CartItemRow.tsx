...
export function CartItemRow({ item, cart }: CartItemRowProps) {
  ...
  return (
    <div className="flex items-center gap-4 py-4 border-b border-slate-100 last:border-0">
      {/* Imagen placeholder */}
      <div className="w-16 h-16 bg-slate-100 rounded-xl flex-shrink-0 overflow-hidden flex items-center justify-center text-slate-300">
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        ...
        <button
          onClick={() => remove({ cartId: cart.id, productId: item.id })}
          disabled={isPending}
